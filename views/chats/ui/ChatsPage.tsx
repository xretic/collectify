...
import ChatListSkeleton from '@/views/chats/ui/ChatsListSkeleton';
...
export default function ChatsPage({ chatId }: ChatPageProps) {
    ...
    const getChats = async (): Promise<void> => {
        ...
        try {
            const data = await chatApi.list(skipRef.current * CHATS_PAGE_LENGTH);
        ...
        const onNew = (msg: SocketMessage) => {
            ...
            setChats((prev) => {
                ...
                const updated: ChatInResponse = {
                    ...
                    unread:
                        msg.userId !== userId && currentActive !== msg.chatId
                            ? (prev[idx].unread ?? 0) + 1
