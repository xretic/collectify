...
import CollectionPageSkeleton from '@/views/collection-details/ui/CollectionPageSkeleton';
...
function applyActionOptimistic(
    ...
    const next: CollectionPropsAdditional & { likesCount?: number; favoritesCount?: number } = {
        ...prev,
    };
...
export function CollectionDetailsPage() {
    ...
    const collectionKey = useMemo(() => collectionQueryKeys.detail(id), [id]);
    const commentsKey = useMemo(() => collectionQueryKeys.comments(id), [id]);
    ...
    const commentsQuery = useInfiniteQuery({
        ...
        initialPageParam: 0,
        queryFn: ({ pageParam }) => collectionApi.getById(id, pageParam),
    ...
    const applyServer = (server: CollectionPropsAdditional) => {
        ...
        queryClient.invalidateQueries({ queryKey: collectionQueryKeys.search });
    };

    const actionMutation = useMutation({
        mutationFn: (action: CollectionActionType) => collectionApi.updateAction(id, action),
    ...
    const commentMutation = useMutation({
        mutationFn: ({ text }: { text: string }) => collectionApi.createComment(id, text),
    ...
    const orderMutation = useMutation({
        mutationFn: async (items: OrderPayloadItem[]) => {
            await collectionApi.updateOrder(id, items);
            return true;
        },
    });
    ...
    const handleFavorite = () => {
        ...
        queryClient.invalidateQueries({
            queryKey: collectionQueryKeys.meSearch,
        });
    ...
    const hasNextCommentsPage = commentsQuery.hasNextPage;
    ...
    const handleLoadMore = useCallback(async () => {
        if (!hasNextCommentsPage || isFetchingNextCommentsPage) return;

        const res = await fetchNextCommentsPage();
        const lastPage = res.data?.pages?.at(-1);
        const next = lastPage?.commentsRes ?? [];
        if (next.length) addComments(next);
    }, [hasNextCommentsPage, isFetchingNextCommentsPage, fetchNextCommentsPage, addComments]);
    ...
    const handleDragEnd = ({ active, over }: DragEndEvent) => {
        ...
        setCollection((prev) => {
            ...
            const oldIndex = prev.items.findIndex((it) => it.id === activeId);
            const newIndex = prev.items.findIndex((it) => it.id === overId);
            if (oldIndex === -1 || newIndex === -1) return prev;

            const moved = arrayMove(prev.items, oldIndex, newIndex);
            const nextItems: CollectionItem[] = moved.map((it, idx) => ({ ...it, order: idx }));

            const payload = nextItems.map((it) => ({ id: it.id, order: it.order }));
